...
function encontrarBackgroundCategoria(nomeCategoria) {
  const pasta = path.join(ROOT, 'uploads', 'background');
  if (!fs.existsSync(pasta)) return '';
  const exts = ['.png', '.jpg', '.jpeg', '.webp'];
  for (const ext of exts) {
    if (fs.existsSync(path.join(pasta, `${nomeCategoria}${ext}`))) {
      return `/uploads/background/${encodeURIComponent(nomeCategoria + ext)}`;
    }
  }
  return '';
}
...
function construirHtml({ produtos, intro, layout, categoria }) {
  ...
  const bgCategoria = encontrarBackgroundCategoria(categoria?.nome || '');
  const bgStyle = bgCategoria ? ' style="background-color: #ffffff"' : '';
  // <img> real em vez de ::before + background-image: iOS renderiza CSS
  // background em pseudo-elementos de PDF de forma inconsistente (flip + zoom).
  const bgImgHtml = bgCategoria
    ? `<img class="intro-bg-img" src="${bgCategoria}" alt="">`
    : '';

  // Substituir {{logo}}, {{bgStyle}} e {{bgImg}} ANTES de aplicar() — caso
  // contrário, aplicar() trata esses placeholders como variáveis ausentes e
  // os remove (e escaparia o HTML do <img>).
  const templateIntroComLogo = templateIntro
    .replace('{{logo}}', logoIntroHtml)
    .replace('{{bgStyle}}', bgStyle)
    .replace('{{bgImg}}', bgImgHtml);
