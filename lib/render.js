...
function construirHtml({ produtos, intro, layout, categoria }) {
  ...
  const bgStyle = bgCategoria
    ? ` style="background-color: #ffffff; --intro-bg: url('${bgCategoria}')"`
    : '';

  // Substituir {{logo}} e {{bgStyle}} ANTES de aplicar() — caso contrário,
  // aplicar() trata esses placeholders como variáveis ausentes e os remove.
